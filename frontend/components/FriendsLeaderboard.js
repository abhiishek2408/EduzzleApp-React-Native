...
const FriendsLeaderboard = ({ navigation }) => {
  ...
  const maxPoints = leaderboard.length ? Math.max(...leaderboard.map(f => f.totalPoints || 0)) : 0;
      ...
      <View className="flex-row mr-8 gap-9">
        {loading ? (
          ...
          <View className="flex-row items-center gap-2">
            <MaterialCommunityIcons name="account-group-outline" size={32} color="#a21caf" style={{ marginLeft: 8 }} />
            <Text className="text-[18px] font-extrabold font-[Inter] bg-gradient-to-r from-violet-700 via-fuchsia-500 to-pink-500 text-transparent bg-clip-text drop-shadow-md tracking-wide">Friends Leaderboard</Text>
          </View>
        )}
        ...
        <View className="px-2 space-y-8">
          {loading ? (
            ...
            <View className="items-center justify-center py-12 rounded-2xl mx-2 shadow-lg border border-purple-200 bg-white">
              <MaterialCommunityIcons name="account-group-outline" size={72} color="#ddd" />
              <Text className="mt-4 text-lg text-gray-400 font-bold">Add friends to see the leaderboard!</Text>
            </View>
          ) : (
            leaderboard.slice(0, 3).map((friend, index) => {
              ...
              return (
                <TouchableOpacity
                  key={friend.userId}
                  activeOpacity={0.9}
                  className={`flex-row items-center rounded-2xl p-3 border-2 shadow-lg bg-white ${marginClass} ${isCurrentUser ? 'border-purple-400' : 'border-transparent'} ${friend.rank <= 3 ? 'border-2' : ''}`}
                >
                  {/* Left: Rank Medal */}
                  <View className="w-11 h-11 items-center justify-center rounded-full bg-white" style={friend.rank <= 3 ? { backgroundColor: getRankColor(friend.rank) + '25' } : {}}>
                    <Text className="text-2xl font-extrabold drop-shadow-md" style={{ color: getRankColor(friend.rank) }}>
                      {getMedalIcon(friend.rank)}
                    </Text>
                  </View>
                  ...
                  <Image
                    ...
                    className={`w-14 h-14 rounded-full ml-3 border-2 shadow-md ${friend.rank <= 3 ? '' : 'border-gray-200'}`}
                    style={friend.rank <= 3 ? { borderColor: getRankColor(friend.rank), borderWidth: 3 } : { borderColor: '#a21caf', borderWidth: 2 }}
                  />
                  ...
                  <View className="flex-1 ml-3 flex-row items-center justify-between">
                    <View className="flex-1">
                      ...
                      <View className="flex-row items-center gap-1">
                        ...
                        <Text className="text-sm font-bold text-[#a21caf]">{friend.quizzesSolved} solved</Text>
                      </View>
                    </View>
                    <View className="items-end bg-white px-3 py-1.5 rounded-xl min-w-[55px] shadow-md">
                      <Text className="text-xl font-extrabold text-purple-600 drop-shadow-md">{friend.totalPoints}</Text>
...
// Styles removed: now using Tailwind utility classes via Nativewind
